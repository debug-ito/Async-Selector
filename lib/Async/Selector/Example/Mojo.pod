...
attached to a piece of resource in the Model layer (in MVC sense), and
its events are published via both Comet (long-polling) and
WebSocket. This example uses L<Mojolicious::Lite> for a Web
application framework because of its great support for real-time
Web. However you can use L<Async::Selector> with any framework because
it completely belongs to the Model layer.
...
part and the HTML/Javascript part. In the MVC paradigm, they
correspond to the Model, Control and View layers, respectively.

=head2 Resource Part (Model)
...
=head2 HTTP Part (Control)

In the HTTP part, the events from C<$selector> are exported to Web
browsers via both Comet (long-polling) and WebSocket. You might have
...
content of C<seq> to C<select()> method on C<$selector> object. That
...
C<$client_sequence>, the callback function given to C<select()> is
...
the server sends a response. That is why the callback function for the
C<select()> method returns 1. This cancels the selection after it
sends the resource to the Web browser.
...
Unlike C</comet>, we call C<select()> method on C<$selector> with the
...
callback function for C<select()> method. We don't have to wait for
the first update of the C<$resource>.

The other reason is that WebSocket connections are persistent.  For
persistent connections, we can let C<$selector> call the callback
function every time the C<$resource> is C<trigger()>-ed.

Because WebSocket connections are persistent, we return 0 in the
callback function for the C<select()> method.  That way, the selection
remains after sending a message to the Web browser.



=head2 HTML/Javascript Part (View)
